...
const V3:React.FC<Props> = ({list,active,isUserExists})=>{
    ...
    <div className="basis">
        ...
        <div className="info">
            ...
            {
                list.map((v,index)=>{
                    const {i, type, isActive, value, count, secondCount, people, reinvest, blocked,placeSource,placeArr} = v;
                           ...
                           <div className="item-pr">
                               ...
                               </div>
                               {
                                   isUserExists?<div className="left-icon">
                                       {blocked?<ExclamationCircleFilled style={{ fontSize: '20px', color: '#c49c42' }} onClick={()=>{
                                           message.warning("您需要激活下一个Level才能继续拿收益",3)
                                       }}/>:
                                       isActive?<CheckCircleFilled style={{ fontSize: '20px', color: '#52c41a' }}/>:<PlusCircleFilled style={{ fontSize: '20px' }} onClick={()=>{active(1,i)}}/>}</div>:""
                               }
                           ...
                           <Row className="circle">
                               {
                                   [0,1,2].map(i=>{
                                       ...
                                       return <Col span={8} className="icon3 text-center">
